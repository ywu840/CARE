...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, dataset_name, max_rank=50):
    """Evaluation with market1501 metric
        Key: for each query identity, its gallery images from the same camera view are discarded.
    """
    
    # print(q_pids)
    # print(np.round(np.diag(distmat)))
    # print(np.amax(distmat, axis=1))
...
class R1_mAP_eval():
    def __init__(self, num_query, dataset_name, max_rank=50, feat_norm=True, reranking=False):
    ...
    def compute(self):  # called after each epoch
        ...
        cmc, mAP = eval_func(distmat, q_pids, g_pids, q_camids, g_camids, self.dataset_name)
