...
def weights_init_kaiming(m):
    ...
    Args:
        m (nn.Module): A PyTorch module whose weights will be initialized.
                      Supports Linear, Conv, and BatchNorm layers.
    
    Notes:
        - For Linear and Conv layers: Uses Kaiming normal initialization
        - For BatchNorm layers: Sets weight to 1.0 and bias to 0.0
        - Bias terms are initialized to 0 for all applicable layers
        - Uses fan_out mode for Linear layers and fan_in for Conv layers
    """
    classname = m.__class__.__name__
    if classname.find('Linear') != -1:    # Linear layers
        nn.init.kaiming_normal_(m.weight, a = 0, mode = 'fan_out')
        nn.init.constant_(m.bias, 0.0)

    elif classname.find('Conv') != -1:    # Convolutional layers
        nn.init.kaiming_normal_(m.weight, a = 0, mode = 'fan_in')
        if m.bias is not None:
            nn.init.constant_(m.bias, 0.0)
    
    elif classname.find('BatchNorm') != -1:    # BatchNorm layers
        if m.affine:    # Check if BatchNorm has learnable parameters
            nn.init.constant_(m.weight, 1.0)
            nn.init.constant_(m.bias, 0.0)

def weights_init_classifier(m):
    """
    Initialize classifier weights using normal distribution.
    
    Args:
        m (nn.Module): A PyTorch module representing a classifier layer.
                      Expected to be a Linear layer.
    
    Notes:
        - Initializes weights using normal distribution with std=0.001
        - Sets bias terms to 0 if they exist
        - Typically used for the final classification layer
        - Smaller initialization helps with training stability
    """
    classname = m.__class__.__name__
...
class TextEncoder(nn.Module):
    def __init__(self, clip_model):
        ...
        self.text_projection = clip_model.text_projection    # Linear projection to text features
