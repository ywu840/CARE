...
def do_train_stage2(cfg,
    ...
    with torch.no_grad():
        ...
        # print(f"\nThe shape of text_features: {text_features.shape}\n")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view) in enumerate(train_loader_stage2):
            ...
            with torch.amp.autocast("cuda", enabled=True):
                ...
                batch_aids = torch.unique(target)
                batch_text_prototypes = text_features[batch_aids]
                # print(target)
                # print(f"batch_aids -- {batch_aids}, batch_text_prototypes.shape -- {batch_text_prototypes.shape}\n")
                loss = loss_fn(score, feat, target, target_cam, logits, batch_text_prototypes, batch_aids)
                # loss = loss_fn(score, feat, target, target_cam, logits, text_features)
            ...
            if cfg.MODEL.DIST_TRAIN:
                if dist.get_rank() == 0:
                    torch.save(model.state_dict(),
                               os.path.join(cfg.OUTPUT_DIR, cfg.MODEL.NAME + '_{}_S{}.pth'.format(epoch, seed)))
            else:
